                    ...
                    if (tagSwatchDetails !== null) 
                    {
                        ...
                        if (strFromServerMassage.indexOf("Error: ") === -1) 
                        {
							...
							*/
							...
							var strIsAvaliable = displayCollectionIsAvaliable(jsonDataFromServer[0].);//holds the the text if the collection is avaliable
							...
							getDocID(arrIndexFabricDetailsHeader[0]).innerHTML = decodeURL(jsonDataFromServer[0].);

							//checks if there is a designer for this colleciion
							if(jsonDataFromServer[0]. !== "")
								//gets the designer's name
								getDocID(arrIndexFabricDetailsHeader[1]).innerHTML = "By " + decodeURL(jsonDataFromServer[0].);

							//checks if there is a summery for this collection
							if(jsonDataFromServer[0]. !== "")
								//gets the summery of the collection
								getDocID(arrIndexFabricDetailsHeader[9]).innerHTML = decodeURL(jsonDataFromServer[0].);

							//checks if the collection is avaliable or not
							if(strIsAvaliable.indexOf('Available Now') === -1) {
								var arrDateParts = jsonDataFromServer[0]..substring(0, jsonDataFromServer[0]..indexOf(" ")).split("/");//holds the parts of the date
							...
							$.each(jsonDataFromServer, function(JSONDataKey, JSONDataValue) {
								...
								if(JSONDataValue[window.localStorage.getItem("strCustomerFullCountry") + ""] === "False")
								{
									//displays the pricing and units
									strSoldOut += "<div class='divFabricPricing'>\n" + 
									"<label class='lblFabricPricing'>$" + JSONDataValue[window.localStorage.getItem("strCustomerFullCountry") + ""] + "</label><br/>\n";

									//add in the dropdown with all of the sizes need to but the amount
									//and sets the defulat of the seleciotn
									strSoldOut += createFabricQTYSelector(parseInt(JSONDataValue[window.localStorage.getItem("strCustomerFullCountry") + ""]), JSONDataValue., 0, 0, "",parseInt("1"));
									
									//closes the select displays the units
									strSoldOut += "<br/>" + 
										"<label class='lblFabricPricing'>" + JSONDataValue[window.localStorage.getItem("strCustomerFullCountry") + ""] + "</label>\n" +
									"</div>\n";
								}//end of if
								else
								...
								tagSwatchDetails.innerHTML += String.format(strTemplateFabricDetails, JSONDataValue., intColorwayID, JSONDataValue[window.localStorage.getItem("strCustomerFullCountry") + ""], strSoldOut, strAddSelectedItemsToCart);
									...
									*/
									...
									switch(JSONDataKey) {
										...
										case "Fabric":
											...
											strInformationPerSections = "<div class='divIndiCollectionDetails'>" +
												"<div class='divCollectionName'>" + 
													"<label>" + JSONInnerDataValue. + "</label>" + 
												"</div>" +
												"<div class='CollectionDesigner'>" + 
													"<label>" + JSONInnerDataValue. +  "</label>" + 
												"</div>" +
												"<div class='CollectionAddedDate'>" + 
													"<label>" + displayCollectionIsAvaliable(JSONInnerDataValue.) +  "</label>" + 
												"</div>" +
											"</div>";
											strLinkToDetails = String.format("getFablicDetails(" + 
												...
												"getDocID(&quot;{10}&quot;), \n" + 
												"[&quot;{11}&quot;]\n" + 
												");", strSearchResultMessage, arrIndexField[8], JSONInnerDataValue., JSONInnerDataValue., JSONInnerDataValue., "aAddSelectedItemsCart", arrIndexField[2], "sectionCheckout", arrIndexField[7], arrIndexField[9], arrIndexField[10], arrIndexFabricDetailsHeader.toString().replace(/,/g,"&quot;,&quot;"));
										break;
									}// end of switch
									...
									tagSearchResultBody.innerHTML += String.format("<div class='divCollectionDetails divFabricDetails customLeft' id='{0}'>\n" +
										...
										"<div class='divCollectionName divFabricSwatchNumber'>\n" +
											"{2}\n" + 
										"</div>\n" + 
									"</div>\n\n", JSONInnerDataValue., JSONInnerDataValue., strInformationPerSections, strLinkToDetails);
